...
class KeyStoreHelper : StorageKeyProvider {
     ...
     * @throws KeyInvalidatedException if the master key was invalidated by the OS
     *   (biometric enrollment change, lock-screen removal, or — empirically — a
     *   stale alias that survived uninstall in invalidated state).
     */
    override fun encryptStorageKey(storageKey: ByteArray): ByteArray {
        try {
            ...
            throw KeyInvalidatedException(
                "Storage key was invalidated by a device security change",
                e,
            )
     ...
     *
     * @throws AuthenticationRequiredException if the device has not been unlocked recently.
     * @throws KeyInvalidatedException if the master key was invalidated, or if the
     *   stored ciphertext no longer authenticates against the current alias.
     */
    override fun decryptStorageKey(encryptedData: ByteArray): ByteArray {
        ...
        try {
            ...
            // Cipher succeeded but the ciphertext didn't authenticate
            // — typically a master/storage-key mismatch after the
            // alias was silently regenerated by the OS while the
            // encrypted blob was preserved. From the caller's view this
            // is the same recovery class as KPIE: the data behind the
            // key is no longer reachable.
            throw KeyInvalidatedException(
                "Stored key cipher could not be authenticated",
                e,
            )
        } catch (e: UserNotAuthenticatedException) {
            throw AuthenticationRequiredException(
...
/**
 * Thrown when the Android KeyStore master key has been invalidated and the
 * stored encrypted blobs that depend on it are no longer recoverable.
 *
 * Triggers: enrolling/removing a fingerprint or face on a key with
 * `setInvalidatedByBiometricEnrollment(true)` (default), removing the
 * device lock screen, or — empirically observed on some devices —
 * surviving uninstall in an already-invalidated state. May also arise
 * from an `AEADBadTagException` when the cached ciphertext was
 * encrypted under a different master-key generation than the alias
 * currently holds.
 *
 * The caller is expected to wipe the local encrypted state (DB +
 * SharedPreferences + alias) and offer the user a recovery path
 * (restore from backup or set up new identity); the data itself is
 * cryptographically lost.
 */
class KeyInvalidatedException(
    message: String,
    cause: Throwable? = null,
) : Exception(message, cause)
