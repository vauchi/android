 ...
 *
 * Launch via ADB:
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es test discovery
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es test aid_select
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es test apdu_latency
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es test max_payload
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es test throughput
 *   adb shell am start -n app.vauchi/.diagnostic.NfcDiagnosticActivity --es mode hce_server
 */
class NfcDiagnosticActivity : ComponentActivity() {
    ...
    private var actionPrompt by mutableStateOf<String?>(null)
    private var actionConfirmed by mutableStateOf(false)
    private var launchedViaAdb = false

    @Volatile private var foregroundTag: Tag? = null
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        // Handle tags from foreground dispatch (Samsung Android 8 fallback)
        @Suppress("DEPRECATION")
        val tag = intent.getParcelableExtra<Tag>(NfcAdapter.EXTRA_TAG)
        if (tag != null) {
            Log.i("Vauchi", "[NFC Diagnostic] Tag from foreground dispatch")
            foregroundTag = tag
        }
        // Handle test re-launch via singleTop
        if (intent.hasExtra("test") || intent.hasExtra("mode")) {
            handleIntent(intent)
        }
    }
    ...
    private fun runTest(test: suspend (MutableList<String>) -> Unit) {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            try {
                ...
                running = false
                // After test completes, prompt to separate devices so NFC field resets
                if (!launchedViaAdb) {
                    promptUser("Separate devices")
                }
            }
        }
    }
